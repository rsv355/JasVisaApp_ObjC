...
@interface AddTravellerViewController ()
{
    MBProgressHUD *hud;
...
@implementation AddTravellerViewController
...
- (void)viewDidLoad {
    ...
    NSLog(@"Traveler DEtail -------->> %@",self.travelerDetailArr);
...
#pragma mark - Design and prefetch data methods
...
-(void)setDataLayout {
    if ([self.travelerDetailArr count] == 0) {
        ...
        imagePathArr = [[NSMutableArray alloc]init];
         _package_id = [self.travelerDetailArr objectAtIndex:2];
...
-(void)saveDataToDatabase {
    ...
    NSString *insertQuery = [NSString stringWithFormat:@"insert into travel_detail (package_id,traveler_type,first_name,last_name,email_id,phone_no,title, nationality_id, nationality_name, special_request) values ('%@','%@','%@','%@','%@','%@', '%@', '%@', '%@','%@')",_package_id,[NSNumber numberWithInteger:0],strFirstName,strLastName,strEmailID,strPhone,strTitle, strNationalityID, strNationalityName, strSpecialRequest];
    NSLog(@"----------------%@",insertQuery);
    ...
    NSLog(@"----------------%@",insertQuery1);
    [self.dbHandler insertDataWithQuesy:insertQuery1];
    [self dismissViewControllerAnimated:YES completion:nil];
    if ([_delegate respondsToSelector:@selector(getBackFromController:)])
    {
        [_delegate getBackFromController:[NSString stringWithFormat:@"%@ %@",self.txtFirstName.text,self.txtLastName.text]];
    }
